...
Horizontal_CollectionViewCellDelegate,
AnchorCollectionViewCellDelegate,
BorasCastCollectionViewCellDelegate,
...
//分类页面发现新奇数组数据
@property (nonatomic, strong) NSMutableArray *arrayOfClassify_List;
//主播
...
- (void)viewDidLoad {
    ...
    self.arrayOfClassify_List = [NSMutableArray array];
    self.arrayOfBroadcast = [NSMutableArray array];
...
- (void)handleDataClassification {
    ...
    [LJNetTool GET:string body:nil headerField:nil response:LJJSON success:^(id result) {
        ...
        [self.arrayOfClassify_DiscoveryColumns addObject:arrayOfList[0]];
        
        NSMutableArray *array = [NSMutableArray arrayWithArray:arrayOfList];
        if (array.count != 0) {
            [array removeObjectAtIndex:0];
        }
        
        NSMutableArray *arrayOfZero = [NSMutableArray array];
        for (NSDictionary *dic in array) {
            LJ_Classify_ListModel *model = [[LJ_Classify_ListModel alloc] initWithDic:dic];
            [arrayOfZero addObject:model];

        }
        
#pragma mark - 双层for循环 (二维数组)
        NSInteger index = arrayOfZero.count / 6;

        for (NSInteger i = 0; i < index; i++) {
 
            NSMutableArray *arrayOfOther = [NSMutableArray array];
            for (NSInteger j = i * 6; j < index * 6; j++) {
                
                [arrayOfOther addObject:arrayOfZero[j]];
 
            }
            [_arrayOfClassify_List addObject:arrayOfOther];

        
        }  if (arrayOfZero.count % 6 != 0) {
            NSMutableArray *arrayOdLast = [NSMutableArray array];
            for (NSInteger i = index * 6; i < arrayOfZero.count; i++) {
                [arrayOdLast addObject:arrayOfZero[i]];
            }
            [_arrayOfClassify_List addObject:arrayOdLast];
...
- (void)handleDataBroadcast {
    NSString *string = @"http://live.ximalaya.com/live-web/v4/homepage?device=iPhone";
    [LJNetTool GET:string body:nil headerField:nil response:LJJSON success:^(id result) {
        NSDictionary *dictionary = (NSDictionary *)result;
        NSDictionary *dic = [dictionary objectForKey:@"data"];
        
        Broadcast *broadcast = [[Broadcast alloc] init];
        [broadcast setValuesForKeysWithDictionary:dic];
//        NSLog(@"%@", dic);
//        NSMutableArray *arrayOfStandby = [NSMutableArray array];
        [self.arrayOfBroadcast addObject:broadcast];
        
//        self.arrayOfBroadcast = arrayOfStandby;
        
        
        NSMutableArray *arrayOfcategories = [dic objectForKey:@"categories"];
        for (NSDictionary *dicOfCategories in arrayOfcategories) {
            LJ_CategoriesModel *model = [[LJ_CategoriesModel alloc] initWithDic:dicOfCategories];
            [self.arrayOfCategories addObject:model];
        }
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.item == 1) {
        ...
        if (self.arrayOfClassify_DiscoveryColumns != 0 && self.arrayOfClassify_List != 0) {
            cell.arrayOfList = _arrayOfClassify_List;
            cell.arrayOfDiscoveryColumns = _arrayOfClassify_DiscoveryColumns;
        }
        ...
        if (indexPath.item == 2) {
            BorasCastCollectionViewCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"cellOfBoradCast" forIndexPath:indexPath];
            

                cell.arrayOfBroadcast = _arrayOfBroadcast;
                
         
                cell.arrayOfCategories = _arrayOfCategories;
...
- (void)UPDataOfHorizontal {
    [_arrayOfAll removeAllObjects];
    [_arrayOfOther removeAllObjects];
...
- (void)UPDataOfAnchor {
    [_arrayOfAnchor removeAllObjects];
    [self handleDataOfAnchor];
}

- (void)UPDataOfBorasCast {

    [_arrayOfBroadcast removeAllObjects];
    
    [_arrayOfCategories removeAllObjects];
    [self handleDataBroadcast];
}

- (void)UPDataOfList {
    [_arrayOfList_Datas removeAllObjects];
    [_arrayOfList_FocusImages removeAllObjects];
    [self handleDataOFList_Datas];
}
